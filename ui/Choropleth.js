import { POWER_TYPES } from "../Constants.js";

export default class Chloropleth {
    constructor(data, options = {}) {
        this._data = data;

        options.container.classList.add("chloropleth");

        let optionsForm = options.container.appendChild(document.createElement("form"));
        optionsForm.addEventListener("submit", this._handleOptionsSubmit.bind(this));

        let yearSelectContainer = optionsForm.appendChild(document.createElement("div"));
        yearSelectContainer.appendChild(document.createElement("span")).textContent = "Year";

        this._yearSelectElement = yearSelectContainer.appendChild(document.createElement("select"));
        this._yearSelectElement.addEventListener("change", this._handleYearChange.bind(this));

        for (let key in data) {
            let optionElement = this._yearSelectElement.appendChild(document.createElement("option"));
            optionElement.selected = key == options.defaultYear;
            optionElement.textContent = key;
        }

        let powerSelectContainer = powerSelectContainer.appendChild(document.createElement("div"));
        powerSelectElement.appendChild(document.createElement("span")).textContent = "Power Type";

        this._powerSelectElement = powerSelectcontainer.appendChild(document.createElement("select"));
        this._powerSelectElement.addEventListener("change", this._handlePowerChange.bind(this));

        // put the "for" loop here
        
        var svg = d3.select("svg"),
        width = +svg.attr("width"),
        height = +svg.attr("height");

        var power = d3.map();

        var path = d3.geoPath();

        var x = d3.scaleLinear()
            .domain([1,10])
            .rangeRound([600, 860]);

        var color = d3.scaleThreshold()
            .domain(d3.range(2, 10))
            .range(d3.schemeOranges[9]);

        var g = svg.append("g")
            .attr("class", "key")
            .attr("transform", "translate(0,40)");

        g.selectAll("rect")
            .data(color.range().map(function(d) {
                d = color.invertExtent(d);
                if (d[0] == null) d[0] = x.domain()[0];
                if (d[1] == null) d[1] = x.domain()[1];
                return d;
            }))
            .enter().append("rect")
                .attr("height", 8)
                .attr("x", function(d) { return x(d[0]); })
                .attr("width", function(d) { return x(d[1]) - x(d[0]); })
                .attr("fill", function(d) { return color(d[0]); })

        g.append("text")
            .attr("class", "caption")
            .attr("x", x.range()[0])
            .attr("y", -6)
            .attr("fill", "#000")
            .attr("text-anchor", "start")
            .attr("font-weight", "bold")
            .text("Power production");

        g.call(d3.axisBottom(x)
                .tickSize(13)
                .tickFormat(function(x, i) { return i ? x : x; })
                .tickValues(color.domain()))
            .select(".domain")
                .remove();

        d3.queue()
            .defer(d3.json, "https://d3js.org/us-10m.v1.json")
            .defer(d3.tsv, this._data, function(d) { power.set(d.id, +d.data); })
            .await(ready);

        function ready(error, us) {
            if (error) throw error;

            svg.append("g")
                    .attr("class", "counties")
                .selectAll("path")
                .data(topojson.feature(us, us.objects.counties).features)
                .enter().append("path")
                    .attr("fill", function(d) { return color(d.data = power.get(d.id)); })
                    .attr("d", path)
                .append("title")
                    .text(function(d) { return d.data; });
            
            svg.append("path")
                .datum(topojson.mesh(us, us.objects.states, function(a, b) { return a !== b; }))
                .attr("class", "states")
                .attr("d", path);
        }
    }

    get _selectedData() {
        return this._data[this._yearSelectElement.value][this._powerSelectElement.value];
    }

    _redraw() {

    }

    _handleOptionsSubmit(event) {
        this._redraw();
    }

    _handleYearChange(event) {
        this._redraw();
    }

    _handlePowerChange(event) {
        this._redraw();
    }
}